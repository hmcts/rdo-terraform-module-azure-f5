...
resource "null_resource" "ansible-runs" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      openssl pkcs12 -in '${path.module}/ansible/files/star-platform-hmcts-net.pfx' -out '/tmp/cert.key' -nocerts -nodes -password pass:Monday01!
      openssl pkcs12 -in '${path.module}/ansible/files/star-platform-hmcts-net.pfx' -out '/tmp/cert.crt' -clcerts -nokeys -password pass:Monday01!
