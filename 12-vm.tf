...
resource "null_resource" "ansible-runs" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      openssl pkcs12 -in '${path.module}/ansible/files/star-platform-hmcts-net.pfx' -out '$(System.DefaultWorkingDirectory)/ansible/files/cert.key' -nocerts -nodes -password pass:Monday01!
      openssl pkcs12 -in '${path.module}/ansible/files/star-platform-hmcts-net.pfx' -out '$(System.DefaultWorkingDirectory)/ansible/files/cert.crt' -clcerts -nokeys -password pass:Monday01!
