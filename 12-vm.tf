...
resource "null_resource" "ansible-runs" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOF
      ...
      ansible-playbook -i ${path.module}/ansible/inventory -vvvvvvv ${path.module}/ansible/f5.yml --extra-vars '{"provider":{"server": "51.145.102.96", "server_port":"443", "user":"local-admin", "password":"hzMCf21DK30pTwa", "validate_certs":"no", "timeout":"300"}}' --extra-vars 'f5_selfip="10.97.0.7"' --extra-vars 'f5_selfsubnet="255.255.255.0"'
     EOF
  }
}